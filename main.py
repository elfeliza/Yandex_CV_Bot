...
async def voice_message_handler(message: types.Message):
    ...
    if text.lower() == "Посмотреть фотографии Элизы".lower():
        await message.answer(
            "Чтобы посмотреть фотографии, проговори в голосовом сообщении одну из команд:\n- Молодая Элиза\n- Старая Элиза")
    elif text.lower() == "Послушать истории".lower():
        await message.answer(
            "Чтобы послушать истории, проговори в голосовом сообщении одну из команд:\n- История любви\n- Об алгоритме обработки естественного языка\n- О базах данных")
...
async def process_callback_button1(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id,
                           "Ты выбрал кнопки для общения со мной. Жмякай, чтобы узнать об Элизе!",
                           reply_markup=urlkb)
